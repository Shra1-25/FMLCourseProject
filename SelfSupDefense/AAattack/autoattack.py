...
class AutoAttack():
    def __init__(self, model, normalize_f, norm='Linf', eps=.3, seed=None, verbose=True,
        ...
        if not self.is_tf_model:
            ...
            self.apgd = APGDAttack(self.model, self.normalize_f, n_restarts=5, n_iter=100, verbose=False,
                eps=self.epsilon, norm=self.norm, eot_iter=1, rho=.75, seed=self.seed, device=self.device)
            
            from .fab_pt import FABAttack_PT
            self.fab = FABAttack_PT(self.model, self.normalize_f, n_restarts=5, n_iter=100, eps=self.epsilon, seed=self.seed,
                norm=self.norm, verbose=False, device=self.device)
        
            from .square import SquareAttack
            self.square = SquareAttack(self.model, self.normalize_f, p_init=.8, n_queries=5000, eps=self.epsilon, norm=self.norm,
            ...
            self.apgd_targeted = APGDAttack_targeted(self.model, self.normalize_f, n_restarts=1, n_iter=100, verbose=False,
                eps=self.epsilon, norm=self.norm, eot_iter=1, rho=.75, seed=self.seed, device=self.device)
    ...
    def get_logits(self, x):
        return self.model(self.normalize_f(x))[0]
